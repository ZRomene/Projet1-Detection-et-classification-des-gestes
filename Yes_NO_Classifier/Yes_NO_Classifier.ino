...
void loop() {
  ...
  // Première question 
  Serial.print("Est-ce que vous allez bien ?");
  Serial.println();
  ...
  if ( (tflOutputTensor->data.f[0]) > (tflOutputTensor->data.f[1]) ) 
    {
        ...
        Serial.println(EMOJIS[0]);
        Serial.println();
    }
  else 
    {
        Serial.print(GESTURES[1]);
        Serial.println(EMOJIS[1]);
        Serial.println();
    }
  Serial.println();
  ...
  // Deuxième question 
  Serial.print("Parlez-vous français ?");
  Serial.println();
  delay(1000);                // waits for a second
  Acquisition ();
  if ( (tflOutputTensor->data.f[0]) > (tflOutputTensor->data.f[1]) ) 
  ...
  // Troisième question 
  Serial.print("Vous utilisez Arduino Nano 33 BLE Sense board ? ");
  Serial.println();
  delay(1000);                // waits for a second
  Acquisition ();
  if ( (tflOutputTensor->data.f[0]) > (tflOutputTensor->data.f[1]) ) 
